...
const RAW_COLLECTION = "districts_raw";
const TARGET_COLLECTION = "districts";
...
async function migrateDistricts() {
    ...
    try {
        ...
        const districtsArray = Object.entries(rawData)
            .filter(([key, value]) => key !== '_id') // loại bỏ _id nếu có
            .map(([_, value]) => value);

        console.log("Tổng số phần tử đọc được:", districtsArray.length);

        if (!Array.isArray(districtsArray)) {
            console.error("districtsArray is not an array");
            return;
        }
        districtsArray.forEach((item, index) => {
        ...
        const validData = districtsArray.filter(item =>
            typeof item === 'object' &&
            !Array.isArray(item) &&
            item !== null &&
