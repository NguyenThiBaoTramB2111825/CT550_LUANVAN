...
const RAW_COLLECTION = "wards_raw";
const TARGET_COLLECTION = "wards";
...
async function migrateWards() {
    ...
    try {
        ...
        const wardsArray = Object.entries(rawData)
            .filter(([key, value]) => key !== '_id') // loại bỏ _id nếu có
            .map(([_, value]) => value);

        console.log("Tổng số phần tử đọc được:", wardsArray.length);

        if (!Array.isArray(wardsArray)) {
            console.error("wardsArray is not an array");
            return;
        }
        wardsArray.forEach((item, index) => {
        ...
        const validData = wardsArray.filter(item =>
            typeof item === 'object' &&
            !Array.isArray(item) &&
            item !== null &&
