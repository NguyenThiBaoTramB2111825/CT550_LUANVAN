  ...
  <div class="container mt-4 justify-content-center">
    <div class="row mx-auto mx-auto pb-5" style="width: 1300px;">
      ...
      <div class="col-md-4 mx-auto" v-if="productDetail.length">
        ...
        <p class="m-0 ">Kích cỡ: </p>
        <div  class="d-flex justify-content-start flex-wrap gap-2">
          <button v-for="size in productDetail[0].sizes" :key="size.id" 
         ...
         <div class="accordion mt-5" id="productInfo">
          <div class="accordion-item">
            ...
            <div id="description" class="accordion-collapse collapse show" data-bs-parent="#productInfo">
              <div class="accordion-body">
                {{productDetail[0].product_desciption }}
              </div>
            </div>
          </div>
          ...
          <div class="accordion-item">
            ...
            <div id="brand" class="accordion-collapse collapse" data-bs-parent="#productInfo">
              <div class="accordion-body">
                {{ productDetail[0].brand_description }}
                {{  productDetail[0].brand_website  }}
              </div>
            </div>
          </div>
          ...
          <div class="accordion-item">
           ...
           <strong>     Về danh mục sản phẩm</strong>
            ...
            <div id="sizeGuide" class="accordion-collapse collapse" data-bs-parent="#productInfo">
              <div class="accordion-body">
                {{ productDetail[0].category_description }}
              </div>
            </div>
          </div>
        </div>
    ...
    const fetchProductDetail = async () => {
      ...
      try {
        ...
        if (product.discount_id) {
          try {
            const discountRes = await axios.get(`${BASE_URL}/api/discount/${product.discount_id}`);
            discount = discountRes.data;
          } catch (error) {
            console.warn("Không thể lấy thông tin discount:", error);
            discount = null; // fallback nếu bị lỗi
          }
        }
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        for (const [productId, details] of productDetailMap.entries()) {
          ...
          const sameBrand = brand.name === productDetail.value.name;
          const sameCate = category.nam === productDetail.value.category_name;

          console.log("Giá trị của sameBrand, sameCate: ", sameBrand, sameCate);
        ...
        console.log("productDetails trong fetchProductDetails : ", productDetails);
        productDetailSelect.value = productDetails.value.filter(p => p.product_id === route.params.id);
